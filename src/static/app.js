    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        participantSection.innerHTML = `
          <h5>Participants</h5>
          ${details.participants.length > 0 ? `<ul>${details.participants
            .map((participant) => `
              <li class="participant-item">
                <span>${participant}</span>
                <button type="button" class="delete-participant" data-activity="${name}" data-email="${participant}">×</button>
              </li>`)
            .join("")}</ul>` : `<p class="no-participants">No participants yet.</p>`}
        `;
